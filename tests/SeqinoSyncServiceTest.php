...
function assertTrue2(bool $condition, string $message): void
{
    if (!$condition) {
...
function assertSame2(mixed $expected, mixed $actual, string $message): void
{
    if ($expected !== $actual) {
...
assertSame2(2, $stats['processed'], 'Should process all outbound rows.');
assertSame2(2, $stats['success'], 'All outbound rows should succeed.');
assertSame2(0, $stats['error'], 'No outbound errors expected.');
assertSame2(array(1, 2), $queue->done, 'Outbound rows should be marked done.');
...
assertSame2(1, $stats2['processed'], 'Should process one e-reporting row.');
assertSame2(0, $stats2['success'], 'E-reporting should fail in this scenario.');
assertSame2(1, $stats2['error'], 'One e-reporting error expected.');
assertTrue2(isset($queue2->errors[10]), 'Error row should be marked in queue.');
